...
*/
;(function (advent) {
	var input = advent.getInputArray(16),
		...
		sueData = input.map(function (sue) {
			var match = sue.match(/Sue (\d+): (\w+): (\d+), (\w+): (\d+), (\w+): (\d+)/i),
				data = { "sue": parseInt(match[1], 10) };

			data[match[2]] = parseInt(match[3], 10);
			data[match[4]] = parseInt(match[5], 10);
			data[match[6]] = parseInt(match[7], 10);

			return data;
		});

	function answer1 () {
		var sueMatch = sueData.filter(function (sue) {
			var isMatch = true;

			Object.keys(sue).forEach(function (key) {
				if (key !== "sue" && sue[key] !== matchData[key]) {
					isMatch = false;
				}
			});
