...
*/
;(function (advent) {
	var rawInput = advent.getInput(19),
	...
	function processInput (raw) {
		...
		raw.split("\n").forEach(function (line) {
			genMatch = line.match(genRegEx);

			if (genMatch !== null) {
				if (!machine.hasOwnProperty(genMatch[1])) {
					machine[genMatch[1]] = [];
				}
				machine[genMatch[1]].push(genMatch[2]);
			} else {
				starting = line;
			}
		});
	...
	function processReplacements (input, debug) {
		var molecules = {},
			molecule,
			i, temp;

		Object.keys(input.machine).forEach(function (key) {
			input.machine[key].forEach(function (replacementValue) {
				temp = input.starting.split(key);

				for (i = 0; i < temp.length-1; i++) {
					molecule = "";
					temp.forEach(function (section, j, arr) {
						if (i === j) {
							molecule += section + replacementValue;
						} else if (j === arr.length - 1) {
							molecule += section;
						} else {
							molecule += section + key;
						}
					});

					if (molecules.hasOwnProperty(molecule)) {
						molecules[molecule]++;
					} else {
		...
		if (debug) console.log(molecules);
		return molecules;
	}
	...
	(function runTests () {
		var testRaw = "H => HO\nH => OH\nO => HH\n\nHOH",
			test = processInput(testRaw),
			a1 = processReplacements(test, 1);
		...
		assert.deepEqual(a1, { "HOOH": 2, "HOHO": 1, "OHOH": 1, "HHHH": 1 });
	})();

	answer1obj = processReplacements(input);
	answer1 = Object.keys(answer1obj).length;

	advent.displayResults(answer1, answer2(input));
