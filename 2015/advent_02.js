/*
	...
	For example:
        ...
        - A present with dimensions 2x3x4 requires 2*6 + 2*12 + 2*8 = 52
		  square feet of wrapping paper plus 6 square feet of slack, for a total of 58
		  square feet.
...
;(function (advent) {
	...
	function Gift (str) {
		...
		this.wrappingPaper = this.dimensions.sides.reduce(function (prevVal, val) {
			return prevVal + (val * 2);
		}, 0) + Math.min.apply(null, this.dimensions.sides);

		this.ribbon = this.dimensions.lengths.reduce(function (prevVal, val) {
			return prevVal + (val * 2);
	...
	input.forEach(function(str) {
		...
		answer1 += giftInfo.wrappingPaper;
